...
# Visium、MERFISH、slideSeq、seqFish、stereoSeq、stereoSeq_h5ad
...
adata = deepen._get_augment(adata, spatial_type="BallTree", use_morphological=True)
...
if 'DeepST_embed' in adata.obsm:
    ...
    plt.title('DeepST潜在表示的UMAP可视化与空间轨迹')
    plt.xlabel('UMAP 1')
    plt.ylabel('UMAP 2')
    plt.tight_layout()
    ...
    ax1.set_title('DeepST潜在表示的UMAP可视化')
            ...
            for domain, count in neighbor_domain_counts.items():
                ...
                if len(domain_indices) > 0:
                    ...
                    if fraction > NEIGHBOR_THRESHOLD:
                        ...
                        print(f"  域 {domain} 是邻近域 (邻居比例: {fraction:.2%})")
