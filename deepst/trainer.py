...
class train():
    def __init__(self,
        ...
        # 为稀疏解码器预处理edge_index (从邻接矩阵提取边索引)
    ...
    def _adj_to_edge_index(self, adj_label):
        """将邻接矩阵转换为edge_index格式 (用于稀疏解码器)"""
        # 处理不同类型的邻接矩阵
        if isinstance(adj_label, torch.Tensor):
            adj_np = adj_label.cpu().numpy()
        else:
            adj_np = adj_label
        
        # 如果是稀疏矩阵，转为密集矩阵
        if hasattr(adj_np, 'toarray'):
            adj_np = adj_np.toarray()
        
        # 提取非零元素的索引
        edges = np.array(np.where(adj_np > 0))
        
        # 转换为PyTorch tensor并移到相应设备
        ...
        with tqdm(total=int(self.pre_epochs), 
            ...
            for epoch in range(self.pre_epochs):
                ...
                if self.domains is None:
                    ...
                    # 稀疏解码器: 传入edge_index，返回边概率 [E]
                    preds = self.model.model.dc(z, self.adj_edge_index)
                ...
                loss = self.model.deepst_loss(
                            ...
                            x=self.data, 
                            preds=preds,        # [E] 稀疏边概率
                            labels=edge_labels,  # [E] 稀疏边标签
                            mu=mu, 
                            logvar=logvar, 
                            n_nodes=self.num_spots, 
                            norm=self.norm, 
                            mask=None,  # 不再需要mask，edge_index已经隐式处理
