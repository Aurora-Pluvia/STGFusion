...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    sparse_mx = sparse_mx.tocoo().astype(
        np.float32)  # 其思想是 按照(row_index, column_index, value)的方式存储每一个非0元素，所以存储的数据结构就应该是一个以三元组为元素的列表List[Tuple[int, int, int]]
    indices = torch.from_numpy(np.vstack((sparse_mx.row, sparse_mx.col)).astype(
        np.int64))  # from_numpy()用来将数组array转换为张量Tensor vstack（）：按行在下边拼接
...
def load_Medata(dataset, npca=20):
    if dataset == 'osmFISH':
        ...
        used_barcode = pd.read_csv(os.path.join('../data/Stereo-seq mouse olfactory bulb/used_barcodes.txt'), sep='\t',
                                   header=None)
...
def load_data(dataset, silce, npca):
    if dataset == "DLPFC":
        ...
        label = pd.read_csv(os.path.join('../data', dataset, silce, silce + '_truth.txt'), sep='\t', header=None,
                            index_col=0)
        label.columns = ['Ground Truth']
        labels = pd.factorize(label["Ground Truth"].astype("category"))[0]
